...
function AdminDashboard() {
  ...
  const [users, setUsers] = useState({ tenants: 0, agents: 0, owners: 0, inquiries: 0 });
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          { label: "Tenants", value: users.tenants },
          { label: "Agents", value: users.agents },
          { label: "Owners", value: users.owners },
          { label: "Inquiries", value: users.inquiries },
      ...
      <div className="bg-white p-4 rounded shadow">
        ...
        {properties.length === 0 ? (
          ...
          <ul className="space-y-2 text-sm">
            {properties.map((p, i) => (
              <li key={i} className="border-b pb-2">
                <p><strong>{p.title}</strong> — Owned by: {p.ownerEmail || "N/A"}</p>
              </li>
            ))}
          </ul>
        )}
