...
function TenantDashboard() {
  ...
  const { name, property, maintenanceRequests = [] } = tenantData;
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-4 rounded shadow">
        ...
        <p><strong>Next Due:</strong> {format(new Date(property.dueDate), "MMM d, yyyy")}</p>
      </div>
      ...
      <div className="bg-white p-4 rounded shadow space-y-2">
        <h2 className="text-lg font-semibold">Rent Status</h2>
        <p>Status: <span className="text-yellow-600 font-medium">Unpaid</span></p>
        <p>Due: {format(new Date(property.dueDate), "MMMM d, yyyy")}</p>
        <a
          ...
          {documents.length === 0 ? (
            ...
            documents.map((doc, i) => (
              <li key={i} className="border-b pb-2">
                <p className="text-sm font-medium text-blue-800">{doc.name}</p>
                <p className="text-xs text-gray-500">{doc.description}</p>
                <p className="text-xs text-gray-400">Uploaded: {format(new Date(doc.uploadedAt), "yyyy-MM-dd")}</p>
                <a
                  href={doc.url}
                  target="_blank"
                  rel="noopener noreferrer"
